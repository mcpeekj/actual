...
function getSchemaMismatchError() {
  return 'This budget could not be loaded because it uses a newer database schema than this version of $quirrel supports. Make sure you are using the latest version, then try again.';
}
...
export function getTestKeyError({ reason }: ErrorWithMeta) {
  switch (reason) {
    ...
    case 'old-key-style':
      return 'This file is encrypted with an old unsupported key style. Recreate the key on a device where the file is available, or use an older version of $quirrel to download it.';
    case 'decrypt-failure':
