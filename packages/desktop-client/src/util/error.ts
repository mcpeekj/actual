...
function getSchemaMismatchError() {
  return t(
    'This budget could not be loaded because it uses a newer database schema than this version of Squirrel supports. Make sure you are using the latest version, then try again.',
  );
}
...
export function getTestKeyError({ reason }: ErrorWithMeta) {
  switch (reason) {
    ...
    case 'old-key-style':
      return t(
        'This file is encrypted with an old unsupported key style. Recreate the key on a device where the file is available, or use an older version of Squirrel to download it.',
      );
    case 'decrypt-failure':
