...
function RenderSimple({ error }: RenderSimpleProps) {
  ...
  if ('IDBFailure' in error && error.IDBFailure) {
    ...
    msg = (
      <Text>
        <Trans>
          ...
          that $quirrel requires to run. This might happen if you are in private
    ...
    msg = (
      <Text>
        <Trans>
          $quirrel requires access to <code>SharedArrayBuffer</code> in order to
    ...
    msg = (
      <Text>
        <Trans>
          $quirrel couldn't load a critical backend worker. Reload the page to try
  ...
  return expanded ? (
    <>
      <Paragraph style={{ marginTop: 10 }}>
        <Trans>
          $quirrel uses <code>SharedArrayBuffer</code> to allow usage from
      ...
      >
        ...
        />{' '}
        <Trans>
          I understand the risks, run $quirrel in the unsupported fallback mode
        </Trans>
      </label>
      <Button
